...
void LinearHashProbeForSD(int key, int hashTable[], int size)
{
    int index = hashValue(key, size);
    if (hashTable[index] != 0)
    {
...
void setDifference(int A[], int sizeofA, int B[], int sizeofB, int hashTable[], int SIZE)
{
    ...
    for (int i = 0; i < sizeofB; i++)
    {
        if(!isPresent(B[i], hashTable, SIZE)){
          LinearHashProbeForSD(B[i], hashTable, SIZE);
        }
    }
    
    for(int i=0; i<sizeofA; i++){
      if(!isPresent(A[i], hashTable, SIZE)){
        flag = 1;
        printf("%d ", A[i]);
      }
    }
